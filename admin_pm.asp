...
if Request.Form("Method_Type") = "newUserPM" then 
	...
	if Err_Msg = "" then
			...
			if (Request.Form("autopmonoff") = "1") then
				...
				strSql = strSql & ",    AUTOPM_MESSAGE = '" & ChkString(Request.Form("automessage"),"SQLString") & "'"
			end if
...
if Request.Form("Method_Type") = "Write_Configuration" then 
	...
	if Err_Msg = "" then
			...
			strMessageDays = datetostr2(dateAdd("d",-Request.Form("strMessageDays"),now()))
	...
	strMessage15Days = datetostr2(dateAdd("d",-split(arrPMadmin1(1),",")(2),now()))
	strMessage30Days = datetostr2(dateAdd("d",-split(arrPMadmin1(2),",")(2),now()))
	strMessage45Days = datetostr2(dateAdd("d",-split(arrPMadmin1(3),",")(2),now()))
	strMessage60Days = datetostr2(dateAdd("d",-split(arrPMadmin1(4),",")(2),now()))
  ...
  <tr>
	...
	<tr valign="top">
	  ...
	  <select name="strMessageDays" size=1>
	  	...
	  	<option value="<%= split(arrPMadmin1(3),",")(2) %>" selected><%= split(arrPMadmin1(3),",")(1) %></option>
...
<input type="hidden" name="Method_Type" value="newUserPM">
	...
	<tr valign="top">
	  <td class="tTitle" align="center" colspan="3"><span class="fTitle"><b><%= txtPMNewMemConf %></b></span></td>
          ...
          <tr valign="top"> 
            <td align="right"><b><%= txtPmAutoNewMem %>:</b>&nbsp;</td>
            <td> 
              <input type="radio" class="radio" name="autopmonoff" value="1" <% if autopmonoff <> 0 then Response.Write("checked")%>><%= txtYES %>&nbsp;(<%= txtOn %>)</td>
            <td> 
              <input type="radio" class="radio" name="autopmonoff" value="0" <% if autopmonoff = 0 then Response.Write("checked")%>><%= txtNO %>&nbsp;(<%= txtOff %>)</td>
          </tr>
          <tr valign="top"> 
            <td align="right"><b><%= txtSubject %>:</b>&nbsp; 
              </td>
            <td colspan="2"> 
              <input class="textbox" type="text" name="autosubjectline" size="20" value="<% if autosubjectline <> "" then Response.Write(autosubjectline) else '## do nothing %>">
              </td>
          </tr>
          <tr valign="top"> 
            ...
            <td colspan="2"> 
              <textarea name="automessage" cols="30" rows="8" wrap="VIRTUAL" class="textbox"><%=automessage%></textarea>
            </td>
          </tr>
          <tr valign="top"> 
