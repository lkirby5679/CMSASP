...
select case sMode
  ...
  case 2 'edit db
	...
	as_start = split(request.Form("START_DATE"),"/")
	a_start = dateserial(as_start(0),as_start(1),as_start(2))
    a_start = datetostr2(a_start)
	as_end = split(request.Form("END_DATE"),"/")
	a_end = dateserial(as_end(0),as_end(1),as_end(2))
    a_end = datetostr2(a_end)
  ...
  case 3 'add db
    'a_start = datetostr2(request.Form("START_DATE"))
    'a_end = datetostr2(request.Form("END_DATE"))
	as_start = split(request.Form("START_DATE"),"/")
	a_start = dateserial(as_start(0),as_start(1),as_start(2))
    a_start = datetostr2(a_start)
	as_end = split(request.Form("END_DATE"),"/")
...
select case iPgType
  case 1 'edit
	...
	if rsAnn.eof then
	  ...
	  s_date = year(s_date) & "/" & doublenum(month(s_date)) & "/" & doublenum(day(s_date))
	  e_date = ChkDate2(rsAnn("A_END_DATE"))
	  e_date = year(e_date) & "/" & doublenum(month(e_date)) & "/" & doublenum(day(e_date))
	  %>
  ...
  case 2 'Add %>
	  ...
	  <tr><td align="right"><b><%= txtStDt %></b></td>
	  <td valign="top"><input type="text" class="textbox" name="START_DATE" id="START_DATE" value="" readonly />&nbsp;<a href="javascript:showCal('Calendar1')"><img border="0" src="images/icons/SmallCalendar.gif" width="16" height="16"></a>
	  </td></tr>
	  <tr><td align="right"><b><%= txtEndDt %></b></td>
	  <td valign="top"><input type="text" class="textbox" name="END_DATE" id="END_DATE" value="" readonly />&nbsp;<a href="javascript:showCal('Calendar2')"><img border="0" src="images/icons/SmallCalendar.gif" width="16" height="16"></a>
	  </td></tr>
  <% 
  If strAllowHtml = 1 Then 
  ...
  case else
	...
	if rsAnn.eof then
	  ...
	  response.Write("<tr><td width=""100%""><hr align=""center"" width=""100%""></td></tr>")
		...
		<img src="images/icons/icon_pencil.gif" border="0" alt="<%= txtEdit %>" title="<%= txtEdit %>" /></a>
    	<a href="javascript:delAnn('<%= rsAnn("A_ID") %>');">
		<img src="images/icons/icon_trashcan.gif" border="0" alt="<%= txtDel %>" title="<%= txtDel %>" /></a>
		&nbsp;<%= isExpired %><br>
