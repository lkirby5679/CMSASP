...
pgType = "manager"
uploadPg = true
if not Request.QueryString("mode") = 1 and not Request.QueryString("mode") = 6  then
...
if Request.QueryString("sort") <> "" or Request.QueryString("sort") <> " " then
	if IsNumeric(Request.QueryString("sort")) = True then
		sorter = Request.QueryString("sort")
	end if
end if 
if Request.QueryString("loc") <> "" or Request.QueryString("loc") <> " " then
...
<% If intMode = "" Then  %>
        <p>
          <%= replace(txtBMgr22,"[%img%]","<img src=""images/icons/icon_trashcan.gif"" width=""12"" height=""12"" border=""0"">") %><br>
		  <%= txtBMgr23 %><br>
          <%= txtBMgr24 %><br>
          <%= txtBMgr25 %></p>
		  <p>
		  <form name="formEle" id="formEle" method="get" action="<%= Request.ServerVariables("URL") %>">
		  <%= txtShow %>:&nbsp;&nbsp;
		  <select name="sort" onChange="submit()">
          	<option value="1"<% If sorter = "" or sorter = "1" Then response.Write(" selected") else 'do nothing %>selected><%= txtALL %></option>
          	<option value="2"<% If sorter = "2" Then response.Write(" selected") else 'do nothing %>><%= ucase(txtActive) %></option>
          ...
          <tr>
            ...
            <table class="tCellAlt2" width="100%" border="0" cellspacing="0" cellpadding="3">
              <tr> 
                ...
                <td align="center" nowrap><b><%= intHits %></b> <%= txtHits %>&nbsp;<a href="admin_banner_manager.asp?mode=3&id=<%= intID %>&loc=<%= loc %>"><img src="images/icons/icon_trashcan.gif" width="12" height="12" border="0" title="<%= txtBMgr26 %>" alt="<%= txtBMgr26 %>"></a></td>
              </tr>
              <tr> 
...
<%   rs.movenext
	  ...
	  if intMode = 2 or intMode = 7 then
		 %>
		 <br>
        ...
        <table width="500" class="tCellAlt2" border="1" cellspacing="0" cellpadding="0" style="border-collapse: collapse;">
          <tr> 
            <td>
              ...
              <input type="hidden" name="memID" value="<%= getMemberID(strdbntusername) %>">
              <input type="hidden" name="loc" value="<%= loc %>">
                  </td>
                </tr>
                <tr> 
...
<% else %><% Response.Redirect "admin_login.asp?target=admin_banner_manager.asp" %><% end if %>
