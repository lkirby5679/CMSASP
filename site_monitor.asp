...
Select Case ActiveSince
	Case "LastVisit" 
		lastDate = ""
	Case "LastHour" 
		lastDate = DateToStr2(DateAdd("h",-1,DateAdd("h", strTimeAdjust , Now())))
	Case "Lastthree" 
		lastDate = DateToStr2(DateAdd("h",-3,DateAdd("h", strTimeAdjust , Now())))
	Case "Lastsix" 
		lastDate = DateToStr2(DateAdd("h",-6,DateAdd("h", strTimeAdjust , Now())))
	Case "Lasttwelve" 
		lastDate = DateToStr2(DateAdd("h",-12,DateAdd("h", strTimeAdjust , Now())))
	Case "LastDay" 
		lastDate = DateToStr2(DateAdd("d",-1,DateAdd("h", strTimeAdjust , Now())))
	Case "Last2Day" 
		lastDate = DateToStr2(DateAdd("d",-2,DateAdd("h", strTimeAdjust , Now())))
	Case "Last3Day" 
		lastDate = DateToStr2(DateAdd("d",-3,DateAdd("h", strTimeAdjust , Now())))
	Case "LastWeek" 
		lastDate = DateToStr2(DateAdd("d",-7,DateAdd("h", strTimeAdjust , Now())))
	Case "LastMonth" 
		lastDate = DateToStr2(DateAdd("m",-1,DateAdd("h", strTimeAdjust , Now())))
	Case Else
		lastDate = ""
End Select
%>
...
if intSubSkin > 0 then
  intSkin = 2
	...
	if mLev < mmLevel Then
		response.Redirect("nowhere.asp")
	end if
...
<form name="LastDateFrm" action="site_monitor.asp" method="post">
  <tr>
    <td align="left">
	&nbsp;Last visit:<br>&nbsp;<%= ChkDate(Session(strCookieURL & "last_here_date")) %> @ <% =ChkTime(Session(strCookieURL & "last_here_date"))%><br>
...
set rsFStatus = my_Conn.Execute (StrSql)
		if chkForumAccess(strUserMemberID,rs("FORUM_ID")) then
			if (mLev = 4) or ((chkForumModerator(rs("FORUM_ID"), Session(strCookieURL & "username"))= "1") and mLev = 3) or ((chkForumModerator(rs("FORUM_ID"), chkString(Request.Cookies(strCookieURL & "User")("Name"),"sqlstring")) = "1") and mlev = 3) or (lcase(strNoCookies) = "1") then
 				AdminAllowed = 1
 			else   
 				AdminAllowed = 0
 			end if
