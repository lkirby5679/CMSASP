...
'~ Change: Skin Levels
'~ Date: 02/06/2006
'~ Change By: wingflap admin@wingflap.com http://www.wingflap.com
'~                                        http://www.planetloser.com
'~
...
' updateskin - Update the Skin Data
	if rfMethod_Type = "updateskin" then
		...
		if FSOenabled = true then
			fPath = Server.MapPath("themes/" & rftxtSkinFolder)
...
<!-----------------Beginning of Skin Info------------------->
     <table width="95%" border="0" cellspacing="0" cellpadding="3">
			  <!-- <tr align="center" valign="middle">
			    <td width="100%" height="25" class="tTitle" align="center" colspan="3">
				  <span class="fTitle"><b><%=txtSknCurDefInfo%></b></span></td>
			  </tr> -->
              <tr align="center" valign="middle"> 
  ...
  <tr>
    <td valign="top">
	  <table border="0" cellspacing="1" cellpadding="1" width="100%">
		...
		<tr align="center" valign="middle">
		  <td>
		  <form name="formSkinLevel" method="post" action="<%= Request.ServerVariables("URL") %>" id="formEle">
...
<%				else
				if rsS("C_SKINLEVEL") = 0 then%>
					<input type="radio" name="tSkinDefault" value="<%=rsS("C_STRFOLDER")%>" onclick="javascript:document['formSkinLevel']['method_type'].value='setdefault';document['formSkinLevel']['method_args'].value='<%=rsS("CONFIG_ID")%>';submit()">
<%				else%>
...
%>
			...
			<td width="20%" class="<%=strCellStyle%>" align="center">
	 		<select class="select" name="tSkinLevel<%=rsS("CONFIG_ID")%>"<%if rsS("C_STRFOLDER") = strDefTheme then response.write(" disabled=" & chr(34) & "disabled" & chr(34))%> onchange="javascript:document['formSkinLevel']['method_type'].value='skinlevel';document['formSkinLevel']['method_args'].value='<%=rsS("CONFIG_ID")%>';submit()">
				<option value="0"<%if rsS("C_SKINLEVEL") = 0 then response.write(" selected=" & chr(34) & "selected" & chr(34))%>><%=txtGuest%></option>
				<option value="1"<%if rsS("C_SKINLEVEL") = 1 then response.write(" selected=" & chr(34) & "selected" & chr(34))%>><%=txtMember%></option>
				<option value="3"<%if rsS("C_SKINLEVEL") = 3 then response.write(" selected=" & chr(34) & "selected" & chr(34))%>><%=txtModerator%></option>
				<option value="4"<%if rsS("C_SKINLEVEL") = 4 then response.write(" selected=" & chr(34) & "selected" & chr(34))%>><%=txtAdmin%></option>
			</select>
			</td>
			<td width="10%" class="<%=strCellStyle%>" align="center">
			<%if rsS("C_STRFOLDER") = strDefTheme then %>
			...
			<tr>
			<td width="100%" valign="top" align="left" colspan="6"><br>
			<div align="justify" class="<%=strCellStyle%>">
				...
				<input type="text" name="txtSkinDesc<%=rsS("CONFIG_ID")%>" id="txtSkinDesc<%=rsS("CONFIG_ID")%>" value="<%=rsS("C_STRDESCRIPTION")%>">
				<input type="hidden" name="txtSubSkin<%=rsS("CONFIG_ID")%>" id="txtSubSkin<%=rsS("CONFIG_ID")%>" value="0"><br>
				</td></tr>
				<!-- <tr><td align="right" width="20%">
				<%=txtSknSubSkinValue%>:&nbsp;
				</td>
				...
				<input type="submit" value="<%=txtSknUpdate%>" id="btnUpdateSkin<%=rsS("CONFIG_ID")%>" name="btnUpdateSkin<%=rsS("CONFIG_ID")%>" onclick="javascript:document['formSkinLevel']['method_type'].value='updateskin';document['formSkinLevel']['method_args'].value='<%=rsS("CONFIG_ID")%>';">
				<!-- <img border="0" style="cursor:hand;" src="images/icons/icon_update.gif" alt="Update <%=rsS("C_TEMPLATE")%>'s data"  onclick="javascript:document['formSkinLevel']['method_type'].value='updateskin';document['formSkinLevel']['method_args'].value='<%=rsS("CONFIG_ID")%>';submit()"> -->
				</td></tr>
				</table>
			</div>
