...
'breadcrumb here
  ...
  if session.Contents("strCGrResult") <> "" then
    showUpdResult(session.Contents("strCGrResult"))
...
<input type="hidden" name="id" value="<%= grpID %>">
	  &nbsp;&nbsp;&nbsp;<a href="javascript:;" onClick="openWindow3('pop_portal.asp?cmd=5&mode=2&cid=<%= grpID %>')"><img src="images/icons/info.gif" alt="<%= replace(txtCG07,"[%groupName%]",grpName) %>" border="0" title="<%= replace(txtCG07,"[%groupName%]",grpName) %>"></a>
	  <% If grpAddMem Then %>&nbsp;<a href="admin_config_groups.asp?mode=1&id=<%= grpID %>"><img src="images/icons/icon_edit_topic.gif" alt="<%= replace(txtCG08,"[%groupName%]",grpName) %>" width="15" height="15" border="0" title="<%= replace(txtCG08,"[%groupName%]",grpName) %>"></a><% End If %>
	  <% If grpActive Then %>&nbsp;<a href="javascript:delGroup('<%= grpName %>','<%= grpID %>');"><img src="images/icons/icon_trashcan.gif" alt="<%= replace(txtCG09,"[%groupName%]",grpName) %>" border="0" title="<%= replace(txtCG09,"[%groupName%]",grpName) %>"></a><% End If %>
</form></td>
    </tr>
	<% 		rsUp.movenext
		loop 
