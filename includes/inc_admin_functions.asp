...
sub ListFolderContents(path)
	...
	if Err.Number=0 then
     set AVfolder = fs.GetFolder(path)
	   ...
	   Else
     Response.Write("<li><A href=""admin_avatar_home.asp?mode=deletefolder&fpath=" & Server.HTMLEncode(AVfolder.name) & """><IMG src=""images/icons/icon_trashcan.gif"" border=""0""></A>&nbsp;&nbsp;<b>" & AVfolder.Name & "</b> - " & AVfolder.Files.Count & " " & txtFile & ", ")
	   End If
     ...
     for each item in AVfolder.SubFolders
		if recurse then
         ...
         Response.Write("<li><A href=""admin_avatar_home.asp?mode=deletefolder&fpath=" & Server.HTMLEncode(item.name) & """><IMG src=""images/icons/icon_trashcan.gif"" border=""0""></A>&nbsp;&nbsp;<b>" & item.Name & "</b> - " & item.Files.Count & " " & txtFile)
     ...
     for each file in AVfolder.Files  '<a href=""javascript:openWindow4('" & url & "');"">
	   ...
	   Response.Write("<li><A href=""admin_avatar_home.asp?mode=deletefile&fpath=" & Server.urlEncode(AVfolder.Name & "\" & file.name) & "&id=" & AVfolder.name & """><IMG src=""images/icons/icon_trashcan.gif"" border=""0""></A>&nbsp;&nbsp;<a href=""" & url & """>" _
         & file.Name & "</a> - " _
         & file.Size & " bytes, " _
