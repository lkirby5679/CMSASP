...
function modGrpEdit(pg,m,c,s,al,ih)
  ...
  modGrpEdit = "<a href=""javascript:;"" onclick=""JavaScript:popUpWind('" & pg & "?mode=" & m & "&amp;cmd=1&amp;cid=" & c & "&amp;sid=" & s & "','groups','430','580','yes','yes');""><img src=""themes/" & strTheme & "/icons/" & gimg & """ title=""" & txtEdGrpAccess & """ alt=""" & txtGrpAccess & """ border=""0"" style=""display:inline;"" hspace=""2""" & algn & "></a>"
end function
...
sub shoGroupAccess(frm,gRead,gWrite,gFull,gLst) %><fieldset style='margin:10px;'>
	  ...
	  <tr><td colspan="2" align="center"><b><%= txtGrpsRead %></b></td></tr>
	  ...
	  <tr><td colspan="2" align="center"><b><%= txtGrpsWrite %></b></td></tr>
  <tr><td align="right" valign="middle" nowrap><a href="JavaScript:moveGroup('Add','<%= frm %>','g_read','g_write');" title="<%= txtCM10 %>"><b><%= txtCM09 %></b></a>&nbsp;&nbsp;<br>
	  ...
	  <tr><td colspan="2" align="center"><b><%= txtGrpsFull %></b></td></tr>
  <tr><td align="right" valign="middle" nowrap><a href="JavaScript:moveGroup('Add','<%= frm %>','g_write','g_full');" title="<%= txtCM10 %>"><b><%= txtCM09 %></b></a>&nbsp;&nbsp;<br>
...
Function chkAppActive(app)
  dim appActive
  appActive = true
  sSQL = "Select APP_ACTIVE from " & strTablePrefix & "APPS WHERE APP_NAME = '" & app & "'"
...
Function chkAppUser(app)
  dim appUser
  appUser = true
  sSql = "SELECT " & strTablePrefix & "APPS.APP_ID, " & strTablePrefix & "APPS.APP_iNAME, " & strTablePrefix & "APPS.APP_ACTIVE, " & strTablePrefix & "APPS.APP_VIEW, " & strTablePrefix & "GROUP_PERMS.G_ID, " & strTablePrefix & "GROUP_PERMS.APP_CAT, " & strTablePrefix & "GROUP_PERMS.APP_SUB, " & strTablePrefix & "GROUP_PERMS.G_LEADER, " & strTablePrefix & "GROUP_PERMS.GP_READ, " & strTablePrefix & "GROUP_PERMS.GP_WRITE, " & strTablePrefix & "GROUP_PERMS.GP_DELETE, " & strTablePrefix & "GROUP_PERMS.GP_MODIFY FROM " & strTablePrefix & "APPS INNER JOIN " & strTablePrefix & "GROUP_PERMS ON " & strTablePrefix & "APPS.APP_ID = " & strTablePrefix & "GROUP_PERMS.APP_ID WHERE (((" & strTablePrefix & "APPS.APP_iNAME)='forums') AND ((" & strTablePrefix & "APPS.APP_ACTIVE)=1));"
  set appChk = my_Conn.execute(sSQL)
  if not appChk.eof then
    if mLev < appChk("APP_USERS") then
      appUser = false
	end if		
  end if
  set appChk = nothing
...
sub updateAccess(tCat,tSub,pag,mde)
...
<%  
  editAccessForm tCat,tSub,pag,mde
end sub

sub editAccessForm(tC,tS,pg,md)
  		  iLoc = 0
  		  bAccess = false
  		  iApp = 0
  		  iCat = 2
  ...
  response.Write("<br />")
	...
	if sid = 0 and cid = 0 and bAppFull then
		...
		p_read = ""
  		cat_name = txtPics
		sub_name = ""
		stMsg = stMsg & cat_name & "&nbsp;" & txtModGrpEdit & "<hr>"
	elseif sid = 0 and cid > 0 then
		sSQL = "select CAT_NAME, CG_READ, CG_WRITE, CG_FULL, CG_INHERIT, CG_PROPAGATE from " & tC & " WHERE CAT_ID=" & cid
  		set rsT = my_Conn.execute(sSQL)
  		cat_name = rsT("CAT_NAME")
		cg_inherit = rsT("CG_INHERIT")
		cg_propagate = rsT("CG_PROPAGATE")
  ...
  '		sub_name = rsT("SUBCAT_NAME")
	...
	if bAccess then
      ...
      <form action="<%= pg %>" method="post" id="GrpAccess" name="GrpAccess" onSubmit="selectUsers('GrpAccess')">
	...
	<%
	  ...
	  if iLoc = 3 then
	    getSelCats(tC)
	    response.Write("<br><br>" & txtSubCatNam & ": <input name=""s_name"" class=""textbox"" type=""text"" value=""" & sub_name & """ maxlength=""40"" style=""margin:0px""><br>")
	  end if
	  'end if
	  'p_read
	  if iLoc > 1 then %>
	   <input id="inherit" name="inherit" type="checkbox" value="1"<%= chkRadio(cg_inherit,1) %>> <%= txtInhPerms %>
   <% End If
...
sub getSelCats(c)
  ...
  sSql = "SELECT CAT_ID, CAT_NAME, CG_READ FROM " & c & " ORDER BY C_ORDER, CAT_NAME"
