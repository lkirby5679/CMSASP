...
if action = "sendemail" then
		...
		stEmail = chkString(Request.Form("Email"),"email")
		'strFrom = strSender
...
if action = "passemail" then
	...
	sendOutEmail strEmail,strsub,strMsg,2,0
  end if
end function
...
strSQL = "SELECT REMINDER_ID, EVENT_ID, EVENT_START, MEMBER_ID, REMINDER_INC FROM " &strTablePrefix & "EVENTS_REMINDERS ORDER BY EVENT_START ASC"
	...
	todayis = CDate(strForumTimeAdjust)
	cdEdate = CDate(Edate)
	adjdate = cdEdate - RemindDiff
	
	if CDate(strForumTimeAdjust) >= CDate(EDate) - RemindDiff then
