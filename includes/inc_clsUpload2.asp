...
sub Upload()
dim f,max,i,name,path,size,success,memID,ext
...
function deleteFile(daPath)
  ...
  If session.Contents("uploadType") = "xxxx" Then
  	uploadType = cint(session.Contents("uploadType"))
	...
	'response.Write("remotePath: " & remotePath)
  	...
  	'sizeLimit = 10000000			         
  	sizeLimit = clng(rsU("UP_SIZELIMIT")*1000) 'maximum size (bytes) of the file to be uploaded
	tmpAllow = rsU("UP_ALLOWEDEXT")
  	if instr(tmpAllow,",") > 0 then
    ...
    'on error resume next
  	...
  	onlyLoggedUsers = rsU("UP_ALLOWEDUSERS") ' 1 = only logged users can upload, 0 = everyone
	active = rsU("UP_ACTIVE") 
	intMaxTW = cLng(rsU("UP_THUMB_MAX_W"))
	intMaxTH = cLng(rsU("UP_THUMB_MAX_H"))
	intMaxW = cLng(rsU("UP_NORM_MAX_W"))
	intMaxH = cLng(rsU("UP_NORM_MAX_H"))
	intResize = cLng(rsU("UP_RESIZE"))
	intDoThumb = cLng(rsU("UP_CREATE_THUMB"))
	'on error goto 0
...
Sub ResizeUploadedFiles(max_path, max_Suffix, max_maxWidth, max_maxHeight, max_Quality, max_RemoveOrig, up_filename)
  ...
  max_maxHeight  = Cint(max_maxHeight)  
	    ...
	    'response.Write("max_newFileName:" & max_newFileName & "<br>")
                FitImage_Comp "ASP.NET", "includes/image_resizer.aspx", Server.MapPath(max_CurPath & max_fileName), Server.MapPath(max_curPath & max_newFileName), max_maxWidth, max_maxHeight, max_Quality
