...
if strIPGateBan = 1 and strIPGateLck = 0 then
			...
			select case rs1("IPLIST_STATUS")
				case "0"
					if ((userdate >= rs1("IPLIST_STARTDATE")) and (userdate <= rs1("IPLIST_ENDDATE"))) then
						...
						if mlev < 3 then call processstatus ("banned")
					end if
				case "1"
					if ((userdate >= rs1("IPLIST_STARTDATE")) and (userdate <= rs1("IPLIST_ENDDATE"))) then
						if strIPGateCok = 1 then call writethecookie ("Watched", ChkDate(rs1("IPLIST_ENDDATE")))
						if strIPGatetype = 0 and strIPGatelog = 1 then call loguser()
						if mlev < 3 then call processstatus ("watched")
					end if
				case "2"
					if ((userdate >= rs1("IPLIST_STARTDATE")) and (userdate <= rs1("IPLIST_ENDDATE"))) then
						pagefound = 0
						...
						do until ((rs3.eof) or (pagefound = 1))
							...
							if lcase(rs3("PAGEKEYS_PAGEKEY")) = lcase(pagename) then
							   ...
							   pagefound = 1
							   if strIPGatetype = 0 and strIPGatelog = 1 then call loguser()
							   if mlev < 3 then call processstatus ("noaccess")
							end if
	...
	if strIPGatecok = 1 and mlev < 3 and FoundIP = 0 and FoundName = 0 then
...
if strIPGateLck = 1 and mlev < 3 and strIPGateBan = 1 then
...
sub autoinsert(message, code)
	...
	strSql = strSql & "values ('" & tempname& "','" & userip & "','" & strCurDateString & "','" & DateToStr2(dateadd("m", 1, date())) & "','" & "Auto inserted: " & message & " cookie detected." & "','" & code & "')"
