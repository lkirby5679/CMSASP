...
%>
  ...
  <select name="action<%= rec %>">
    <option value="0" selected>--[<%= txtSelect %>]--</option>
...
sub DropDownPaging(fnum)
	if maxpages > 1 then
		...
		if fnum = 1 then
			Response.Write("<b>" & txtPage & ": </b><select name=""whichpage"" size=""1"" onchange=""ChangePage(" & fnum & ");"">" & vbNewLine)
		else
			Response.Write("<b>" & replace(txtPendMem3,"[%count%]",maxpages) & ": </b><select name=""whichpage"" size=""1"" onchange=""ChangePage(" & fnum & ");"">" & vbNewLine)
		end if
