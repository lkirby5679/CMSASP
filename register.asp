...
elseif Request.QueryString("mode") <> "DoIt" and Request.QueryString("actkey") = "" then
	if strNewReg = "0" and mLev < 4 then
...
elseif Request.QueryString("actkey") <> "" and lcase(strEmail) = 1 then
	...
	if (Instr(Request.Form("Name"), ">") > 0 ) or (Instr(Request.Form("Name"), "<") > 0) or (Instr(Request.Form("Name"), ",") > 0) or (Instr(Request.Form("Name"), "&") > 0) or (Instr(Request.Form("Name"), "#") > 0) or (Instr(Request.Form("Name"), "'") > 0) or (Instr(Request.Form("Name"), "(") > 0) or (Instr(Request.Form("Name"), ")") > 0) or (Instr(Request.Form("Name"), ";") > 0) or (Instr(Request.Form("Name"), ":") > 0) then
		Err_Msg = Err_Msg & "<li>" & txtCharsNotAllow & "</li>"
	else
		strSql = "SELECT M_NAME FROM " & strMemberTablePrefix & "MEMBERS WHERE " & strDBNTSQLName & " = '" & Trim(chkString(Request.Form("Name"),"sqlstring")) &"'"
		...
		if not len(Request.Form("Referrer")) > 0 then	
			'## Do Nothing
		else			
			'
			...
			if rs.BOF and rs.EOF then 
				Err_Msg = Err_Msg & "<li>" & txtBadRefer & ".</li>"
			else 
				'## Do Nothing
			end if
			rs.close
			...
			if Trim(Request.Form("Name")) = Trim(Request.Form("Referrer")) then
				Err_Msg = Err_Msg & "<li>" & txtReferNotU & ".</li>"
			else 
				'## Do Nothing
			end if
		end if
	end if 'name field check
  ...
  if showRegisterLongForm = 1 then
	...
	if Err_Msg = "" then
	  ...
	  if showRegisterLongForm = 0 then
		strSql = "INSERT INTO " & strMemberTablePrefix 
		if (lcase(strEmail) = "1" and (strEmailVal = 5 or strEmailVal = 6 or strEmailVal = 7 or strEmailVal = 8)) or ((Request.Form("reservation") = "yes" or Request.Form("active") = 0 or Request.Form("sendinvite") = "yes") and mlev = 4)  then
		...
		strSql = strSql & ", " & "'" & ChkString(Request.Form("Name"),"sqlstring") & "'"
		strSql = strSql & ", " & "'" & ChkString(pEncrypt(pEnPrefix & chkString(Request.Form("Password"),"sqlstring")),"password") & "'"
		...
		if lcase(strEmail) = "1" and (strEmailVal = 5 or strEmailVal = 6 or strEmailVal = 7 or strEmailVal = 8) and Request.Form("reservation") <> "yes" then
			strSql = strSql & ", " & "-1"
		elseif (Request.Form("reservation") = "yes" or Request.Form("sendinvite") = "yes") and mlev = 4 then
			strSql = strSql & ", -7"
		else
			strSql = strSql & ", 1"
		end if	
		...
		if (Request.Form("reservation") = "yes" and mlev = 4) then
			strSql = strSql & ", " & "0"
		else
			strSql = strSql & ", 1"
		end if
		...
		strSql = "INSERT INTO " & strMemberTablePrefix 
		if (lcase(strEmail) = "1" and (strEmailVal = 5 or strEmailVal = 6 or strEmailVal = 7 or strEmailVal = 8)) or ((Request.Form("reservation") = "yes" or Request.Form("sendinvite") = "yes") and mlev = 4)  then
			strSql = strSql & "MEMBERS_PENDING "
		else
			strSql = strSql & "MEMBERS "
		end if
...
'		on error resume next
	...
	'	on error resume next
	  ...
	  'send the emails as needed	
		...
		if strEmail = "1" and Request.Form("sendinvite") = "yes" and mLev = 4 then
	...
	call postRegTextPM()
		...
		if strAuthType = "db" and mLev = 0 then
		  select case getMlev(chkString(Request.Form("Name"),"sqlstring"), pEncrypt(pEnPrefix & chkString(Request.Form("Password"),"sqlstring")))
			case 1, 2, 3, 4
...
'<!-- ::::::::::::::::::::::::: start BASICS info ::::::::::::::::::::::::::::: --> %>
      ...
      <% if trim(Request.QueryString("rname") <> "") then %>
        <tr> 
          <td width="40%" align="right" nowrap><b><%= txtRefer %>:&nbsp;</b></td>
      ...
      <%
		if strAuthType = "nt" then %>
          <tr> 
            <td align="right" nowrap valign="top">
          	...
          	<td> 
            <%if mlev = 4 then %>
            <input name="Account" value="<%= Session(strCookieURL & "userid") %>" size="20">
            <%else %>
        ...
        <tr> 
          <td align="right" nowrap><b><span class="fAlert"><b>*</b></span> 
            <%= txtPass %>:&nbsp;</b></td>
          <td> 
		...
		%>
        <tr> 
          <td align="right" nowrap><span class="fAlert"><b>*</b></span><b><%= txtEmlAdd %>:&nbsp;</b></td>
          <td>
            <INPUT name="Email" size="25" maxlength="90" value="">
            </td>
        </tr>
        <tr> 
          <td align="right" nowrap><b><span class="fAlert">*</span><%= txtCfmEml %>:&nbsp;</b></td>
          <td>
		...
		if mlev = 4 then %>
        <!-- <tr> 
          <td align="right" nowrap><b><%= txtMlev %>:&nbsp;</b></td>
          <td> 
            <SELECT name="Level">
              <option value="1" selected><%= txtNmlUsr %></option>
              <option value="2"><%= txtModerator %></option>
              <option value="3"><%= txtAdminst %></option>
            </SELECT>
          </td>
        </tr>
        <tr> --> 
          <td align="right" nowrap><b><%= txtActive %>:&nbsp;</b></td>
          <td> 
            <SELECT name="active">
              <option value="1" selected><%= txtActive %></option>
	  ...
	  %>
	<tr><td nowrap align="center" valign="middle" colspan="2">&nbsp;
<%  If SecImage > 0 Then %>
	<br><%= txtEntrSecImg %><br>
	<img align="absolute" src="<%= strHomeUrl %>includes/securelog/image.asp" /><br>
	<input CLASS="textbox" type="text" name="secCode" size="8" maxLength="8" value="">
<%  end if %>
	</td>
	</tr>
...
'::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
sub showErrMsg()
		if (InStr(chkString(Request.ServerVariables("HTTP_REFERER"), "refer"), "policy.asp") <> 0) or (Err_Msg <> "") then 
%>
			<p align=center><span class="fTitle"><%= txtThereIsProb %></span></p>
			<table align=center border=0>
			  <tr>
...
<%		
			...
			<p align=center><span class="fTitle"><%= txtThereIsProb %></span></p>
			<p align=center><%= replace(replace(txtRegKey12,"[%a%]","<a href=""policy.asp""><u>"),"[%/a%]","</u></a>") %>.</p>
<%		
		end if %>
    		<!--#INCLUDE file="inc_footer.asp" --><%
		Response.End
...
<%		  else	
		    newUser = ChkString(Request.Form("Name"),"sqlstring")
			'response.Write("<br><br>name: " & newUser & "<br>")
			'response.Write("auth: " & strEmailVal & "<br><br>")
		    call sendPMtoNewUser(newUser)
		  end if
	end if
	if (Request.Form("reservation") = "yes" and mlev = 4) then %>
