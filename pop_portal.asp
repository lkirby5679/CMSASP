...
'if not hasPerm then
  select case iPgType
    ...
    case 2 'lock member
	  if mLev = 4 and iMode = 1 and c_id > 0 then
	    lockMember(c_id)
	  elseif mLev = 4 and iMode = 0 and c_id > 0 then
...
sub delMember(m_id) 
	intPostcount = 0
	intReplycount = 0
	memberName = getMemberName(c_id)
	'######### Delete bookmarks ###########
	strSql = "DELETE FROM " &strTablePrefix & "BOOKMARKS WHERE M_ID = " & c_id
	executeThis(strSql)
	
	'######### Delete Subscriptions ###########
	strSql = "DELETE FROM " &strTablePrefix & "SUBSCRIPTIONS WHERE M_ID = " & c_id
	executeThis(strSql)
	
	'######### Delete front page user ###########
	strSql = "DELETE FROM " &strTablePrefix & "FP_USERS WHERE fp_uid = " & c_id
	executeThis(strSql)
	
	'######### Delete from groups ###########
	strSql = "DELETE FROM " &strTablePrefix & "GROUP_MEMBERS WHERE G_MEMBER_ID = " & c_id
	executeThis(strSql)
	
	'######### Delete cp config ###########
	strSql = "DELETE FROM " &strTablePrefix & "CP_CONFIG WHERE MEMBER_ID = " & c_id
	executeThis(strSql)
	
	'######## Delete personal avatar folder, if there is one. ##################
	if FSOenabled = true then
		remotePath = "\members\" & c_id
	...
	strSql = strSql & "WHERE M_TO = " & c_id & " OR M_FROM = " & c_id & ";"
	executeThis(strSql)
  ...
  if chkApp("links","USERS") then
	...
	strSql = "SELECT LINK, RATING FROM LINKS_RATING WHERE RATE_BY = " & c_id & ";"
	...
	if not rs.eof then
	  do until rs.eof
	    ...
	    rs.movenext
	  loop
	  strSql = "DELETE FROM LINKS_RATING WHERE RATE_BY = " & c_id & ";"
	  executeThis(strSql)
	end if
	set rs = nothing
	'######### Update articles ###########
  ...
  if chkApp("downloads","USERS") then
	'######### Delete article rating ###########
	strSql = "SELECT DL, RATING FROM DL_RATING WHERE RATE_BY = " & c_id & ";"
	set rs = my_Conn.Execute(strSql)
	if not rs.eof then
	  do until rs.eof
	    ...
	    rs.movenext
	  loop
	  strSql = "DELETE FROM DL_RATING WHERE RATE_BY = " & c_id & ";"
	  executeThis(strSql)
	end if
	set rs = nothing
	'######### Update articles ###########
  ...
  if chkApp("article","USERS") then
	'######### Delete article rating ###########
	strSql = "SELECT ARTICLE, RATING FROM ARTICLE_RATING WHERE RATE_BY = " & c_id & ";"
	set rs = my_Conn.Execute(strSql)
	if not rs.eof then
	  do until rs.eof
	    ...
	    rs.movenext
	  loop
	  strSql = "DELETE FROM ARTICLE_RATING WHERE RATE_BY = " & c_id & ";"
	  executeThis(strSql)
	end if
	set rs = nothing
	'######### Update articles ###########
  ...
  if chkApp("pictures","USERS") then
	...
	strSql = "SELECT PIC, RATING FROM PIC_RATING WHERE RATE_BY = " & c_id & ";"
	set rs = my_Conn.Execute(strSql)
	if not rs.eof then
	  do until rs.eof
	    ...
	    rs.movenext
	  loop
	  strSql = "DELETE FROM PIC_RATING WHERE RATE_BY = " & c_id & ";"
	  executeThis(strSql)
	end if
	set rs = nothing
	'######### Update pictures ###########
  ...
  if chkApp("forums","USERS") then
	...
	strSql = strSql & " WHERE " & strTablePrefix & "MODERATOR.MEMBER_ID = " & c_id & ";"
	executeThis(strSql)
	' - Remove the member from forum 'allowed members' table
	strSql = "DELETE FROM " & strTablePrefix & "ALLOWED_MEMBERS "
	strSql = strSql & " WHERE " & strTablePrefix & "ALLOWED_MEMBERS.MEMBER_ID = " & c_id & ";"
	executeThis(strSql)
	
	'######### get topic count #########
	strSql = "SELECT COUNT(T_AUTHOR) AS POSTCOUNT "
	strSql = strSql & "FROM " & strTablePrefix & "TOPICS "
	strSql = strSql & "WHERE T_AUTHOR = " & c_id & ";"
	set rs = my_Conn.Execute(strSql)
	if not rs.eof then
	...
	strSql = strSql & "WHERE R_AUTHOR = " & c_id & ";"
	set rs = my_Conn.Execute(strSql)
	if not rs.eof then
	...
	if ((intReplycount + intPostCount) = 0) then
		...
		strSql = strSql & " WHERE " & strMemberTablePrefix & "MEMBERS.MEMBER_ID = " & c_id & ";"
		executeThis(strSql)
	else					
		'######### disable account 
...
sub lockMember(m_id)
  if instr(strWebMaster,"" & lcase(getMemberName(c_id)) & ",") <> 0 then
...
<% ElseIf iMode = 1 Then  ' list groups to add to modules 
	...
	for (i=0; i<oObj[tObj].length; i++) {
	  ...
	  document.write('<img src="images/icons/info.gif" alt="" border="0"></a>&nbsp;');
	  //document.write('');
...
sub selectMemAllow(memID,memName)%>
      <tr>
        <td><a href="javascript:;" onClick="openWindow3('pop_portal.asp?cmd=5&mode=2&cid=<%= memID %>')"><img src="images/icons/info.gif" alt="<%= replace(txtCG07,"[%groupName%]",memName) %>" border="0" title="<%= replace(txtCG07,"[%groupName%]",memName) %>"></a>&nbsp;
...
<%	case 2 'AIM %>
	...
	case 3 'msn
	  ...
	  <b><%= chkString(Request.QueryString("msn"),"sqlstring") %></b><br><br>
	  <%= txtRemThe %>&nbsp;"[no-spam]"<br></td></tr>
