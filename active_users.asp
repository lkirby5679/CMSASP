...
if request.Cookies(strTempCookieType & "Reload") <> "" then
	nRefreshTime = cLng(request.Cookies(strTempCookieType & "Reload"))
end if

if Request.form("cookie") = "1" then
    Response.Cookies(strTempCookieType & "Reload").Path = strCookieUrl
	Response.Cookies(strTempCookieType & "Reload") = chkString(request.Form("RefreshTime"),"sqlstring")
	Response.Cookies(strTempCookieType & "Reload").expires = strCurDateAdjust + 365
...
'breadcrumb here
  arg1 = txtActvUsrs & " (" & txtLstUpdd & "&nbsp;" & strForumTimeAdjust & ")|active_users.asp"
  arg2 = ""
  arg3 = ""
  arg4 = ""
  arg5 = ""
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <select name="SelectedViewType" size="1" onchange="document.frmSelectViewType.submit();">
		<option value="0" <% if ViewType="0" then Response.Write(" SELECTED")%>><%= txtViewMem %> Only</option>
		<option value="1" <% if ViewType="1" then Response.Write(" SELECTED")%>><%= txtViewGst %> Only</option>
		<option value="2" <% if ViewType="2" then Response.Write(" SELECTED")%>><%= txtViewAll %></option>
	</select>
    ...
    <select name="RefreshTime" size="1" onchange="autoReload();">
        <option value="0" <% if nRefreshTime = "0" then Response.Write(" SELECTED")%>><%= txtNoRld %></option>
        <option value="1" <% if nRefreshTime = "1" then Response.Write(" SELECTED")%>><%= txtRld1m %></option>
        <option value="5" <% if nRefreshTime = "5" then Response.Write(" SELECTED")%>><%= txtRld5m %></option>
        <option value="10" <% if nRefreshTime = "10" then Response.Write(" SELECTED")%>><%= txtRld10m %></option>
        <option value="15" <% if nRefreshTime = "15" then Response.Write(" SELECTED")%>><%= txtRld15m %></option>
...
<table cellpadding="1" cellspacing="0" border="0" width="100%">
  <tr>
    <td class="tSubTitle" align="center" valign="top" nowrap><b><%= txtUsrName %></b></td>
<%	if (mlev = 4 or mlev = 3) then %>
    <td class="tSubTitle" align="center" valign="top" nowrap><b><%= txtIPaddy %></b></td>
<%	end if %>
	<td class="tSubTitle" align="center" valign="top" nowrap><b><%= txtLstAct %></b></td>
    <td class="tSubTitle" align="center" valign="top" nowrap><b><%= txtOnlTime %></b></td>
</tr>
<%
...
%>
  <tr class="<% =CColor %>">
<%  if Right(rs("UserID"), 5) = txtGuest then %>
    <td valign="middle" align="center"><%= tmpRobot %></td>
 <% else %>
...
%>
 <% end if 
	if mlev = 4 or mlev = 3 then %>
	<td valign="middle" align="center"><a href="http://ws.arin.net/cgi-bin/whois.pl?queryinput=<%=rs("UserIP")%>" target="_blank"><% =rs("UserIP")%></a></td>
<%	end if
	if lcase(rs("UserID"))= strSiteOwner and not lcase(strDBNTUserName) = strSiteOwner then
	LastUserAction = txtInvis
	else
	LastUserAction = replace(rs("M_BROWSE"),"&rsquo","'")
	end if
%>
    <td valign="middle" align="center"><% =LastUserAction%></a></td>
    <td align="center" valign="middle" width="100" nowrap><%=strOnlineTotalTime%></td>
  </tr>
...
<% if maxpages > 1 then %>
	<table border="0" width="100%">
  	<tr>
    	<td valign="top" align="center"><b><%= txtPages %>: </b>
    	<% Call Paging() %></td>
  	</tr>
	</table>
<% end if %>
