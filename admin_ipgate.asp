...
Case "deletelog"
	Set rs = Server.CreateObject("ADODB.Recordset")
	StrIPGateExp = datetostr2(dateAdd("d",-StrIPGateExp,now()))
...
Case "editipupdate"
	...
	Set rs = Server.CreateObject("ADODB.Recordset")
			...
			strSql = strSql & " SET IPLIST_MEMBERID = '" & memberid & "', IPLIST_STARTIP = '" & startip & "', IPLIST_STARTDATE = '" & DateToStr2(startdate) & "', IPLIST_ENDDATE = '" & DateToStr2(enddate) & "', IPLIST_COMMENT = '" & usercomment & "', IPLIST_STATUS = '" & userstatus & "' "
...
Case "addipupdate"
	...
	Set rs = Server.CreateObject("ADODB.Recordset")
			...
			strSql = strSql & "values ('" & memberid & "','" & startip & "','" & DateToStr2(startdate) & "','" & DateToStr2(enddate) & "','" & usercomment & "','" & userstatus & "')"
			rs.Open StrSql, strConnString 
			if rs.State = 1 then rs.Close
			qry =""
...
strIPGateArchive=datetostr2(DateAdd("d",-strIPGateArchive,now()))
Set rs = Server.CreateObject("ADODB.Recordset")
