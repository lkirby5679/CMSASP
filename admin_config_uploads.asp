...
<form name="form2" id="form2" method="post" action="admin_config_uploads.asp">
 ...
 <% set rsUp = my_Conn.execute("select * from " & strTablePrefix & "UPLOAD_CONFIG")
    ...
    <tr align="center" class="tCellAlt0">
      <td><a href="admin_config_uploads.asp?mode=1&loc=<%= cLng(rsUp("ID")) %>"><img src="images/icons/icon_edit_topic.gif" title="<%= txtEdit %>" alt="<%= txtEdit %>" width="15" height="15" border="0"></a></td>
      ...
      <!-- <td>
	  <% select case chkString(rsUp("UP_ALLOWEDUSERS"),"display")
	  	   case 5
		     Response.Write(txtSAdmin)
	  	   case 4
		     Response.Write(txtAdmin)
		   case 3
		     Response.Write(txtModerator)
		   case 1
		     Response.Write(txtMember)
		   case else
		     Response.Write(txtEveryone)
		 end select%>
		</td> -->
...
sub displayForm(typ)
	if typ = "edit" then
		...
		if not rsUp.eof then
			...
			'strLvl = cLng(rsUp("UP_ALLOWEDUSERS"))
