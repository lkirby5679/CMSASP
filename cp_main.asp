...
<%If rs.EOF or rs.BOF then %>
      <tr>
        <td colspan="5"><b><%= txtNoTopFnd %></b></td>
      </tr>
<%else
	currForum = 0 
	fDisplayCount = 0 
...
<%			end if %>
			...
			<td valign="center"><a href="forum_topic.asp?TOPIC_ID=<% =rs("TOPIC_ID") %>&amp;FORUM_ID=<% =rs("FORUM_ID") %>&amp;CAT_ID=<% =rs("CAT_ID") %>&amp;Topic_Title=<% =ChkString(left(rs("T_SUBJECT"), 50),"urlpath") %>&amp;Forum_Title=<% =ChkString(rs("F_SUBJECT"),"urlpath") %>"><% =ChkString(left(rs("T_SUBJECT"), 50),"display") %></a>&nbsp;</td>
			<td valign="center" align="center"><% =rs("T_REPLIES") %></td>
			<td valign="center" align="center"><% =rs("T_VIEW_COUNT") %></td>
			<%
...
<%		rs.MoveNext 
	...
	if fDisplayCount = 0 then %>
		  <tr><td colspan="6"><b><%= txtNoTopFnd %></b></td></tr>
<%
	end if 
 end if
...
if flag_showrecenttopics > 0 then 
  if strRecentTopics = "1" then
	strStartDate = DateToStr2(dateadd("d", -30, now()))
	...
	if rs2.EOF or rs2.BOF then  %>
 	  <tr><td colspan="5">&nbsp;<br><%= txtNoTopFnd %>...<br></td></tr><%
	else 
	  currTopic = 0
	  TopicCount = 0				
...
<%	 		  end if
			...
			&nbsp;</td>
  			<td valign="center"><a href="forum_topic.asp?TOPIC_ID=<% =rs2("TOPIC_ID") %>&FORUM_ID=<% =rs2("FORUM_ID") %>&CAT_ID=<% =rs2("CAT_ID") %>&Topic_Title=<% =ChkString(left(rs2("T_SUBJECT"), 50),"urlpath") %>&Forum_Title=<% =ChkString(rs2("F_SUBJECT"),"urlpath") %>"><% =ChkString(left(rs2("T_SUBJECT"), 50),"display") %></a></td>
			<td align="center"><%= rs2("T_REPLIES") %></td>
			<td align="center"><%= rs2("T_VIEW_COUNT") %></td>
			<%
  ...
  if not rsT.eof then
    ...
    do until rsT.eof
	  ...
	  if rsT("G_GROUP_LEADER") = 1 or hasAccess(1) then
	    ...
	    Response.Write("<img src=""images/icons/icon_pencil.gif"" alt=""" & txtEdit & """ title="""" border=""0"" hspace=""4"" /></a>")
	  end if
...
sub showPreferences()
 Select Case Request("mode")
  ...
  Case "PMoptions"
	...
	showUpdResult(txtPMSetUpd)

  Case "BirthdayX"
	strSQL = "UPDATE " & strTablePrefix & "MEMBERS SET "
	strSQL = strSQL & "M_SHOW_BIRTHDAY='1' "
	strSQL = strSQL & "WHERE MEMBER_ID=" & id
	executeThis(strSQL)
	'response.write ("bday should be set to 1")
	if chkApp("events","USERS") then
	    sSQL = "INSERT INTO " & strTablePrefix & "EVENTS (Start_Date, End_Date, Event_Title, Event_Details, Date_Added, Added_by, PRIVATE, SERIES, PENDING) Values ('" & DateToStr2(Request.Form("START_DATE")) & "', '" & DateToStr2(Request.Form("END_DATE")) & "',' " & chkstring(Request.FORM("EVENT_TITLE"), "sqlstring") & "', '" & chkstring(Request.FORM("EVENT_DETAILS"), "message") & "', '" & strCurDateString & "', " & chkString(Request.Form("memberid"),"sqlstring") & "," & 0 & "," & -1 & "," & 0 & ")"
	    executeThis(sSQL)

    	Set RS = my_Conn.Execute("SELECT @@IDENTITY") 
    	'###### The following variable will store the EVENT_ID # of the record we just inserted #####
    	idOfNewlyAddedRecord = RS(0)
    	RS.Close
    	set rs = nothing
			
		'Write the Reminder Record		
		sql = "INSERT INTO " & strTablePrefix & "Events_Reminders (EVENT_ID,EVENT_START,MEMBER_ID,REMINDER_INC) VALUES (" & idOfNewlyAddedRecord & "," & datetostr(Request.Form("START_DATE")) & "," & chkString(Request.Form("memberid"),"sqlstring") & ",0)"
 	 	executeThis(sql)

		showUpdResult(txtBDayAdCal)
	 else
		showUpdResult("")
	 end if
 end select
  ...
  spThemeBlock1_open(intSkin)
	showDateTimeOpts()
    if flag_PMstatus then
	  showPMOptions()
	end if
	if chkApp("eventsX","USERS") then
	  showBdaySettings()
	end if
	if flag_HasForums then
	  showDisplayOpts()
...
sub showBdaySettings()
  strSQL = "SELECT * FROM " & strTablePrefix & "MEMBERS WHERE MEMBER_ID=" & id
  Set objRS3 = my_Conn.Execute(strSQL) %>
<table class="tPlain" width="100%">
<tr><td class="tSubTitle" align="center"><b><%= txtBDaySet %></b></td></tr>
<%If IsDate(objRS3("M_AGE")) Then %>
<%  if objRS3("M_SHOW_BIRTHDAY")="0" then %>
<tr><td align="center">
<Form method="POST" action="cp_main.asp?cmd=5" name="BirthdaySet">
<br />
<%= txtUBDayIs %>: <% =chkDate(datetostr(objRS3("M_AGE")))%>
<br />
<%  ' Put in check for next birthday here, add 1 to year from birthdate until it exceeds today's date
	'DispFirstDayWeek = (DateAdd("d",-TodayWeek+1,dDispWeek))
	'
	todayis = strCurDateAdjust
	birthvardate = objRS3("M_AGE")
	yearsold = 0
	
	Do until CDate(birthvardate) >= CDate(strCurDateAdjust)
	  birthvardate = (DateAdd("yyyy",1,birthvardate))
	  yearsold = yearsold + 1
	  'response.write("birthvardate is : " & birthvardate)
	Loop
	'response.write "loop end " & birthvardate
%>
<%= txtUNxtBdayIs & " " & chkDate(datetostr(birthvardate)) %>
<br />
<input type="hidden" name="mode" Value="Birthday">
<input type="hidden" name="start_date" value="<%=birthvardate%>">
<input type="hidden" name="end_date" value="<%=birthvardate%>">
<input type="hidden" name="event_title" value="<%=objRS3("M_NAME") & "'s " & txtBDay %>!">
<input type="hidden" name="event_details" value="<%=objRS3("M_NAME") & " " & txtIs & " " & yearsold%>!<br><%= txtHapBDayFrm & " " & strSiteTitle%>">
<input type="hidden" name="memberid" value="<%=id%>">
<input type="submit" value="<%= txtAddBDayCal %>" class="button"> <br />
...
<td><p align="center">&nbsp;
<br />
<%= txtNoBDayInProf %>, <a href="cp_main.asp?cmd=9"><%= txtEditProf %></a> <%= txtToAddBDay %>. <br />
&nbsp;</p>
</td>
</tr>
...
if lcase(strDBNTUserName) = "skydogg" then
 'showDateTimeTest() 
  ...
  <tr>
    <td align="right"><b><%= txtServLCID %>:</b>&nbsp;</td>
    <td>
    <b><%= session.LCID %></b>
    </td>
  </tr>
  <tr>
    <td align="right"><b><%= txtPortTime %>:</b>&nbsp;</td>
    <td><% portDate = chkDate2(strCurDateString) & chkTime2(strCurDateString) %>
	<b><%= portDate %></b>
  ...
  <tr>
    <td align="right"><b><%= txtMemTime %>:</b>&nbsp;</td>
    <td>
  ...
  <tr>
    <td align="center" colspan="2"><hr></td>
	  ...
	  %>
  <tr>
    <td align="right"><b><%= txtMemLCID %>:</b>&nbsp;</td>
    <td>
	<% displayLCID() %>
  ...
  <tr>
    <td align="right"><b><%= txtTimeDisp %>:</b>&nbsp;</td>
    <td>
    ...
    <a href="JavaScript:openWindow3('pop_help.asp?mode=2&this=1#timetype')"><img src="images/icons/icon_smile_question.gif" border="0"></a>
    </td>
  </tr>
  <tr>
    <td align="right"><b><%= txtTimeAdj %>:</b>&nbsp;</td>
    <td>
    ...
    <a href="JavaScript:openWindow3('pop_help.asp?mode=2&this=1#TimeAdjust')"><img src="images/icons/icon_smile_question.gif" border="0"></a>
    </td>
  </tr>
</table><br />
...
%>
	<Form method="GET" action="cp_main.asp?cmd=5">
		...
		<INPUT TYPE="RADIO" NAME="emailstorage" VALUE="0" <% if rsPmOpts("M_PMEMAIL") = "0" then Response.Write("checked")%>>&nbsp;<%= txtNoPMNoto %>.</td>
		  ...
		  <tr>
		    <td><% if flag_pm_layout= 3 then flag_pm_layout= 0 %>
		      <b><%= txtInOutPref %></b><br />
...
sub showDateTimeTest() %>
  <tr valign="top">
	...
	response.Write("strForumTimeAdjust:<br>" & strForumTimeAdjust & "<br>")
	response.Write("strForumDateAdjust: " & strForumDateAdjust & "<br>")
	response.Write("long date: " & FormatDateTime(strForumTimeAdjust,1) & "<br>")
	
		%></td>
    	<td align="left"><% 
	tmpStrDateTime = datetostr2(now())
	'tmpStrDateTime = ""
	tmpTime = "12:50:36 PM"
	...
	response.Write("<br>last_here_date: " & Session(strCookieURL & "last_here_date") & "<br>")
	
		%></td>
    	<td align="left"><% 
	timeAdj = 0
	CkxTime = DateAdd("h", timeAdj , now())
	tmpStrDateTime = datetostr2(CkxTime)
