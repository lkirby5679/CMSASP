...
if lcase(strDBType) = "access" then
 if request("cmd")="1" and FSOenabled then
  ...
  response.Write("<hr>")
  ShowForm
 elseif request("cmd")="2" and FSOenabled then
   if intIsSuperAdmin then
   ...
   response.Write("<hr>")
   ShowForm
 elseif request("cmd")="3" and FSOenabled then
   if intIsSuperAdmin then
	 eFile = request("db")
     Dim fso
...
sub ShowForm
 ...
 <%
	...
	'Response.Write("recursing folder<br>")
	    ...
	    bDate = strToDate(split(bDate,"_")(1))
	    Response.write("<div style=""padding:2px;"">")
	    Response.write("<a href=""admin_db.asp?cmd=3&db=" & f.ParentFolder & "\" & x.Name & """ title=""" & txtDel & """>")
        Response.write("<img src=""images/icons/icon_trashcan.gif"" border=""0"" title=""" & txtDel & """></a>&nbsp;")
