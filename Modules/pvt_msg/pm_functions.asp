...
%>
        <tr>
          <td class="tSubTitle" colspan="<% if (strShowModerators = "1") or (mlev = 4 or mlev = 3) then Response.Write("7") else Response.Write("6")%>">
            <b><%= txtPvtMessgs %>: </b></td>
        </tr>
        <tr>
          ...
          <td valign=top colspan="<% if (strShowModerators = "1") or (mlev = 4 or mlev = 3) then Response.Write("7") else Response.Write("5")%>"><A HREF="pm.asp"><%= txtPMinbox %></A><BR>
...
function GetPMToday()
  dim rs, intFemales, intMales 
  set rs = server.CreateObject("adodb.recordset")
  strSql = "SELECT COUNT(M_ID) AS PMCount FROM " & strTablePrefix & "PM WHERE m_sent > '" & DateToStr2(DateAdd("h",-24,DateAdd("h", strTimeAdjust , Now()))) & "'"
  rs.Open strSql, my_Conn
...
if typ = 0 or typ = 3 then %>
	  <tr><td></td><td  vAlign="top" align="left">(<%= txtSepWthComm %>)<br>&nbsp;</td></tr>
      <tr>
        <td noWrap vAlign="top" align="right"><b><%= txtSndTo %>:</b></td>
        <td><input name="sendto" value="<%= memName %>" size="45">&nbsp;&nbsp;<a href="JavaScript:pmmembers();"><%= txtMbrLst %></a>
	<br><%if (mLev = 4) then %><input type="checkbox" name="allmem" value="true"><%= txtSndPMAllMem %><% Else %><input type="hidden" name="allmem" value="false"><%end if%>
        <br>&nbsp;</td>
      </tr>
<% else %>
...
if typ > 0 then
  ...
  <%
	...
	Response.Write "        <td valign=""top"" class=""tCellAlt0"" width=""150"" nowrap>"
	Response.Write "><b>" & ChkString(rs("M_NAME"),"display") & "</b></td>" & vbCrLf
	Response.Write "        <td class=""tCellAlt0"" valign=""top"">"
...
sub sendPM(typ)
  ...
  elseif typ = 3 and boolSend = true then
	...
	if Request.form("allmem") = "true" and mlev = 4 and typ = 0 and boolSend = true then
...
function DeleteBanSelection()
{
  ...
  <tr><td align="right" style="padding:2px;">
	...
	<td align="left" style="padding:2px;">
	<% 'allow member 'save' folder %>
	  <select name="iDATA6">
	    <option value="1"<%= CheckSelected(iDATA6,1) %>><%= txtAllMem %></option>
	    <option value="3"<%= CheckSelected(iDATA6,3) %>><%= txtModAdmin %></option>
	    <option value="4"<%= CheckSelected(iDATA6,4) %>><%= txtAdminOnly %></option>
	  </select>
    </td>
  </tr>
  <tr valign="middle"> 
  ...
  <tr valign="middle"> 
    <td align="center" colspan="2" style="padding:2px;"><%= txtMemNoPMAcces %></td>
  </tr>
        <tr><td align="center" valign="middle"><a href="JavaScript:pmBanMembers();" title="Add member to blacklist"><b><%= txtAddMem %></b></a><br>
				<a href="JavaScript:DeleteBanSelection();" title="Remove selected members(s) from this blacklist"><b><%= txtRemMem %></b></a></td>
          <td align="center"><p><b><%= txtBlkdMem %></b><br>
            <select size="5" name="BlockedUsers" style="width:170;" multiple>
			  <%= getBlockedUsers() %>
			  <option value="0"></option>
            </select>
            <input type="hidden" name="hasBlockedUsers" value="1"><br>&nbsp;</p>
          </td>
        </tr>
<%
end sub
...
function pmBlacklist(membID)
  ...
  <tr><td  valign="top" width="100%"><%= txtMemNotAllPM %><hr align="center"></td></tr>
<%
  ...
  <tr> 
	...
	<img border="0" src="themes/<%= strTheme %>/icons/icon_block.gif" align="middle"> 
	<% else %>
	<img border="0" src="themes/<%= strTheme %>/icons/icon_block.gif" align="middle"> 
	<% end if %>
...
<%
  ...
  <tr> 
    <td  valign="top" width="100%" align="center">&nbsp;<b><%= txtEmpty %></b></td>
  </tr>
<%
	end if
  end if
  set rsBL = nothing%>
  <tr> 
    <td  valign="top" width="100%" align="center"><hr><b><%= txtClkMemToRem %></b></td>
  </tr>
<%
...
spThemeBlock1_open(intSkin)
  inBxCnt = pmInboxCount()
%>
<table border="0" cellpadding="0" cellspacing="1" width="100%">
  <tr> 
    <td  valign="top" width="100%">&nbsp;<img border="0" src="images/icons/icon_sent_items2.gif" align="middle" width="15" height="15"> 
  ...
  <tr> 
    <td  valign="top" width="100%">
	  <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr> 
          <td>&nbsp;<img border="0" src="images/icons/icon_inbox2.gif" align="middle" width="15" height="15">&nbsp;<a href="pm.asp"><%= txtPMinbox %></a>&nbsp;(<span class="fAlert"><%= clng(pmCount) %></span>)
          </td>
        </tr>
        <tr> 
          <td> 
  ...
  <tr> 
    <td valign="top" width="100%"> 
	  ...
	  <% If pm_out = 1 Then %>
        <tr> 
          <td>&nbsp;<img border="0" src="images/icons/icon_outbox2.gif" align="middle" width="15" height="15">&nbsp;<a href="pm.asp?cmd=1"><%= txtPMoutBx %> </a> 
...
<% if iPMsaveFolder and mLev >= iPMfolderAccess then %>
  <tr> 
    <td  valign="top" width="100%">
	  <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr> 
          <td>&nbsp;<img border="0" src="images/icons/icon_inbox2.gif" align="middle" width="15" height="15">&nbsp;<a href="pm.asp?cmd=7"><%= txtSvdItems %></a>&nbsp;(<span class="fAlert"><%= clng(pmSavedCount) %></span>)
          </td>
        </tr>
      </table>
    </td>
